...
| QCNN v1  | 15% test samples, 10 epochs, 128 batch size, 1 qubits, 1 layers, filter size = [3, 3], stride = [1, 1], followed by classical head [8, 2], classical preprocessing = standard scaling | 190 | +/- 0.73 |
...
| QCNN v2  | 15% test samples, 10 epochs, 128 batch size, 2 qubits, 2 layers, filter size = [2, 2], stride = [2, 1], followed by classical head [8, 2], classical preprocessing = standard scaling | 194 | +/- 0.68 |
